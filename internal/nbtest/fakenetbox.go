...
// Package nbtest provides an in-memory NetBox API for tests. It began life as
// the reconciler package's own test helper; it is promoted here so that any
// package needing to drive a real *client.NetBoxClient against a fake server —
// the reconciler, and now the importer — shares one faithful fake rather than
// each maintaining its own.
//
// It supports filtered list, get-by-ID, create, partial update, and delete on
// any /api/{app}/{endpoint}/ path, and records every mutating request so tests
// can assert exactly what a caller did.
//
// Like real NetBox, creating or deleting a cable sets or clears the "cable"
// reference on the terminated ports — the cable reconciler's idempotency
...
// FakeNetBox is an in-memory NetBox API for tests.
...
// RecordedRequest is one mutating request the fake received.
...
// New starts a fake server and connects a real NetBoxClient to it. NewClient
// ensures the managed tag against the fake, so the mutation log is cleared
// afterwards: tests only see their own traffic.
func New(t *testing.T) (*FakeNetBox, *client.NetBoxClient) {
	t.Helper()
...
// URL returns the fake server's base URL, for callers that build their own
// client (e.g. to assert a read-only import issues no writes).
func (f *FakeNetBox) URL() string { return f.srv.URL }
...
func (f *FakeNetBox) handle(w http.ResponseWriter, r *http.Request) {
	...
	case r.Method == "OPTIONS":
		// A minimal OPTIONS response: no POST action, so the client's schema
		// parser yields an empty (unconstrained) schema. Enough for callers
		// that probe an endpoint's schema without asserting on it.
		json.NewEncoder(w).Encode(map[string]interface{}{"actions": map[string]interface{}{}})
...
// ResetMutations clears the recorded mutation log.
